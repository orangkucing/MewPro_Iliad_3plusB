...
// For Time Lapse
// generating periodic shutter by using Timer1 Compare Match B interrupt

volatile int8_t lapse;
int8_t rate = 0;

// interrupt interval 1 second
#define TIMER1_COUNTS (16000000UL / 1024UL) // 16MHz. prescaler divisor 1024
...
void StartTimerInterrupt(int r)
{
  ...
  // Timer 1
  // COM1B[1:0] = 0 : OC1B pin disconnected
  // WGM1[3:0] = 12 : Clear Timer on Compare Match (CTC) Mode 12
  TCCR1B = _BV(WGM13) | _BV(WGM12); // set no clock source
  TCCR1A = 0;
  // the following registers can be set properly only after WGMn is set
  ICR1 = TIMER1_COUNTS - 1;       // TOP (1s)
  TCNT1 = 0;
  TIFR1 = _BV(OCF1B);   // clear output compare C match flag
  TIMSK1 = _BV(OCIE1B); // output compare C match interrupt enable
  lapse = 0;
  interrupts();
  TCCR1B |= _BV(CS12) | _BV(CS10); // CS1[2:0] = 5 (internal clock source. prescaler divisor 1024)
